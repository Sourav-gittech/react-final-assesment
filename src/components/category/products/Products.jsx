...
import { Col, Container, Row, Button, Card, Form, InputGroup, Image } from 'react-bootstrap'
import { FaCartPlus } from 'react-icons/fa'
import { IoListCircleSharp } from 'react-icons/io5'
import { Link } from 'react-router-dom'
import axiosInstance from '../../../api/axiosInstance/axiosInstance'
import { endPoint_Product, endPoint_User } from '../../../api/api_url/apiUrl'
  ...
  if (products.length === 0) {
    return (
      <div className='maintain_gap'>
        <h2 className="text-center mt-3">No Products Found.</h2>
        <Image src='/assets/category/category.gif' alt='#product' />
      </div>
    )
  }
                ...
                <Col lg={3} md={4} sm={12} key={product.id}>
                  <Card className='mt-3'>
                    ...
                    <Card.Body>
                      ...
                      <div className="text-center product-control">
                        {!checkLoggedUser() ?
                          (<Button variant="outline-danger m-1 product_btn" onClick={() => addToCart(product.id, product.stock_quantity, product.min_quantity, product.max_quantity)}><FaCartPlus className='icon' /> Add to cart</Button>) : null}
                      </div>
                    </Card.Body>
                  </Card>
                </Col>
